...
namespace xRanges
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            xMenu = new Menu("〖L#中文社区〗攻击范围", "xRanges", true);

            
            xMenu.AddSubMenu(new Menu("范围显示", "Drawing"));
            xMenu.SubMenu("Drawing").AddItem(new MenuItem("DrawER", "显示 敌方 范围").SetValue(true));
            xMenu.SubMenu("Drawing").AddItem(new MenuItem("DrawAR", "显示 队友 范围").SetValue(true));
            xMenu.SubMenu("Drawing").AddItem(new MenuItem("DrawAA", "显示 自己 范围").SetValue(true));
