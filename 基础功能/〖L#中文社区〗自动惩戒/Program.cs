...
namespace meta_Smite
{
    class Program
    {
        ...
        private static void Game_OnGameStart(EventArgs args)
        {
            ...
            setSmiteSlot();
           Config = new Menu("〖L#中文社区〗惩戒", "metaSmite", true);
            Config.AddItem(new MenuItem("Enabled", "启用切换").SetValue(new KeyBind("N".ToCharArray()[0], KeyBindType.Toggle, true)));
            Config.AddItem(new MenuItem("EnabledH", "启用保存 ").SetValue(new KeyBind("K".ToCharArray()[0], KeyBindType.Press)));
            ...
            {
                Game.PrintChat("Smite and spell not found, disabling Meta Smite 鍔犺級鎴愬姛 鏇村璁块棶www.loll35.com");
                return;
            }
            setupCampMenu();
