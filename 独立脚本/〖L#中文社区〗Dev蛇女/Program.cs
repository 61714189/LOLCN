...
namespace DevCassio
{
    class Program
    {
        ...
        private static void InitializeMainMenu()
        {
            ...
            Config = new Menu("〖L#中文社区〗Dev蛇女", "DevCassio", true);
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRSaveYourself", "使用 R保护自己").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRSaveYourselfMinHealth", "使用R保护最低生命值").SetValue(new Slider(25, 0, 100)));

            Config.AddSubMenu(new Menu("骚扰", "Harass"));
            Config.SubMenu("Harass").AddItem(new MenuItem("HarassToggle", "启用骚扰 (自动)").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Toggle)));
            ...
            Config.AddSubMenu(new Menu("防止突进", "Gapcloser"));
            Config.SubMenu("Gapcloser").AddItem(new MenuItem("RAntiGapcloser", "使用R防止突进").SetValue(true));
            Config.SubMenu("Gapcloser").AddItem(new MenuItem("RInterrupetSpell", "使用R中断法术").SetValue(true));
            ...
            Config.SubMenu("Legit").AddItem(new MenuItem("LegitCastDelay", "使用E延迟").SetValue(new Slider(500, 0, 1500)));

            Config.AddSubMenu(new Menu("R 设置", "Ultimate"));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseAssistedUlt", "使用大招支援").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("AssistedUltKey", "强制R 键位").SetValue((new KeyBind("R".ToCharArray()[0], KeyBindType.Press))));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("BlockUlt", "防止 空大").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseUltUnderTower", "塔下自动R敌人").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UltRange", "使用 R 范围").SetValue(new Slider(700, 0, 850)));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("RMinHit", "使用 R 人数").SetValue(new Slider(2, 1, 5)));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("RMinHitFacing", "使用 R 人数（正对)").SetValue(new Slider(1, 1, 5)));
