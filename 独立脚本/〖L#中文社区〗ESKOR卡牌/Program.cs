...
namespace TwistedFate
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("〖L#中文社区〗卡牌大師", "TwistedFate", true);
            ...
            /* Q */
            var q = new Menu("Q - 萬能牌", "Q");
            q.AddItem(new MenuItem("AutoQI", "自動Q 不動的").SetValue(true));
            q.AddItem(new MenuItem("AutoQD", "自動Q 移動的").SetValue(true));
            q.AddItem(new MenuItem("CastQ", "使用Q (半自動)").SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(q);

            /* W */
            var w = new Menu("W - 選牌", "W");
            w.AddItem(
                new MenuItem("SelectYellow", "選擇 黃牌").SetValue(new KeyBind("W".ToCharArray()[0],
                    KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectBlue", "選擇 藍牌").SetValue(new KeyBind("E".ToCharArray()[0], KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectRed", "選擇 紅牌").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(w);

            var r = new Menu("R - 命運", "R");
            r.AddItem(new MenuItem("AutoY", "R後自動切黃牌").SetValue(true));
            Config.AddSubMenu(r);

            var misc = new Menu("雜項", "Misc");
            misc.AddItem(new MenuItem("PingLH", "敵人殘血自動R(限本地)").SetValue(true));
            ...
            /*Drawing*/
            var Drawings = new Menu("範圍", "Drawings");
